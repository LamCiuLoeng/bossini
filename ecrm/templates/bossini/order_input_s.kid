...
<div style="width:1200px;">
	...
	<!--  main content begin -->	
	<form action="/bossinipo/saveStyleLabelOrder" method="POST">
		<input type="hidden" name="orderType" value="S"/>
		        ...
		        <tr>
		          ...
		          <td class="STYLE19 color-FFFFFF">
					<select name="item" id="item"  style="width:300px">
						<option value=""></option>
						<option py:for="item in items" value="${item.id}" itemCode="${item.itemCode}" rmbprice="${item.rmbPrice}" hkprice="${item.hkPrice}">${item.itemCode}(${item.component})</option>
					</select>
				  </td>
		        </tr>
		        <tr>
