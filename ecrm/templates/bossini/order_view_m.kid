...
<div style="width:1200px;">
	...
	<!--  main content begin -->		
		    ...
		    <tbody><tr>
		      <td class="color-000000"><table width="100%" cellspacing="1" cellpadding="0" border="0">
		        ...
		        <tr>
		          ...
		          <td class="STYLE19 color-FFFFFF"><span>${mlInfo.item.itemCode}</span>&nbsp;</td>
		        </tr>
		        
		        <?python
		        	totalQty = sum([d.qty for d in mlInfo.getDetails()])
		        ?>
		        
		        <tr>
		        	<td class="STYLE18 color-CCFFCC">Reference Total Order Price (before VAT)<br/>参考税前单价（运费除外）:</td>
		        	<td class="STYLE19 color-FFFFFF">
		        		<div id="amt_div" py:if="order.currency=='HKD'" py:strip="">&nbsp;$ ${mlInfo.item.hkPrice} <b>X</b> ${totalQty} <b>=</b> $ ${"%.6f" %(totalQty*mlInfo.item.hkPrice)}</div>
		        		<div id="amt_div" py:if="order.currency=='RMB'" py:strip="">&nbsp;￥ ${mlInfo.item.rmbPrice} <b>X</b> ${totalQty} <b>=</b> ￥ ${"%.6f" %(totalQty*mlInfo.item.rmbPrice)}</div>
		        	</td>
		        </tr>
		        <tr>
		          ...
		          <td class="STYLE19 color-FFFFFF">
		          	<table cellspacing="0" cellpadding="0" border="0">
		          		<tr py:for="lc in  mlInfo.getDetails()">
		...
		<div style="float:left;width:200px;margin-left:50px;margin-top:15px">
			<div><a href="#itemImg" class="photo"><img class="hangTagImg" src="/static/images/bossini/${mlInfo.item.itemCode}.jpg" width="190"/></a></div>
			<div id="itemImg"><img class="hangTagImg" src="/static/images/bossini/${mlInfo.item.itemCode}.jpg"/></div>
		</div>
