...
function checkInput(){
	...
	$(".legacy-code").each(function(){
		var t = $(this);
		if(t.val()){
			isFillLegacyCode = true;
			var p = t.parents("tr")[0];
			if(!$("input[name^='qty']",p).val()){ fillCorrect = false; }
		}
	});
	
	if(!isFillLegacyCode){ msg.push("请选择或者输入至少一个 Legacy Code!") }
	if(!fillCorrect){ msg.push("请输入相应 Legacy Code 的订购数量！") }
...
function addLegacy(obj){
	...
	$("input[name^='legacyCode']",cp).attr("name","legacyCode"+index).val("");
	$("input[name^='qty']",cp).attr("name","qty"+index).val("");
