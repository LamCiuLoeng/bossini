...
if __name__ == "__main__":
    ...
    for row in range(beginRow,endRow+1):
        data.append( {
                ...
                "mainLabel" : uniStr(sheet.Cells(row,5).Value),
                "sizeLabel" : uniStr(sheet.Cells(row,6).Value),
                "hangTag" : uniStr(sheet.Cells(row,7).Value),
                "cat":uniStr(sheet.Cells(row,8).Value)
                })
